...
class RefinerScaledUNetModel(models.BaseModel):
  ...
  def create_model(self, model_input, is_training=True, scope="refiner", **unused_params):
    """Build a U-Net architecture
    Args:
        X (4-D Tensor): (N, H, W, C)
        is_training (1-D Tensor): Boolean Tensor is required for batchnormalization layers
    ...
    """
    self.scope = scope
    down_sample = FLAGS.scaled_unet_downsample_rate

    print "model_input", model_input
    model_input = tf.pad(model_input, paddings=[[0,0], [0,0], [1,1], [0,0]])
    print "padded model_input", model_input

    model_input = self.downsampling_2D(model_input, (down_sample, down_sample))
    with tf.variable_scope(self.scope):
      predictions = self.unet(model_input, is_training, **unused_params)
    predictions = self.upsampling_2D(predictions, "prediction", (down_sample, down_sample))
